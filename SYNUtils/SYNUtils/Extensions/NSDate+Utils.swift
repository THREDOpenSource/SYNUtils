...
extension NSDate: Comparable {
    static var isoFormatter: NSDateFormatter {
        // NSDateFormatter is not thread-safe, so we allocate per thread
        return lazyThreadLocalObject("SYNUtils.NSDate.isoFormatter") {
    ...
    static var rfc3339Formatter: NSDateFormatter {
        // NSDateFormatter is not thread-safe, so we allocate per thread
        return lazyThreadLocalObject("SYNUtils.NSDate.rfc3339Formatter") {
            let formatter = NSDateFormatter()
